...
def apply_splices(vcxproj: Path, filters: Path | None, splices: list[Splice],
    ...
    for sp in splices:
        if f'Include="{sp.win_path}"' in new:
            ...
            continue
        opener = re.compile(
            re.escape(f'<{sp.kind} Include="') + re.escape(CONTENT_ANCHOR) + r'[^"]*"'
        )
    ...
    if filters is not None:
        flog = _apply_filters(filters, splices, dry_run=dry_run)
        log.extend(flog)
    return log
...
def _apply_filters(filters: Path, splices: list[Splice], *, dry_run: bool) -> list[str]:
    log: list[str] = []
