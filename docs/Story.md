...
- 島は、底に埋まった **浮遊石** の力で浮いている。拠点の島には緑・光・火の **3つ** の浮遊石があり、
  3つそろって「島の心臓」と呼ばれる。石は砕けたのではなく、**丸ごと抜けて飛んでいった**（欠片・核片という設定はやめた。2026-09-24）。
...
5. 墜ちたドラゴンが島の中心に爪を突き立て、**3つの浮遊石が地面から抜け出して**三方の狩り場へ飛んでいく。
   島がぐらりと傾いて暗転する。
   ...
   爪（`Attack1`。ここで島全体を上から映す `LookDestroyIslandCamera` に切り替え、地割れを見せる）→ `IslandHeartBreak`（光の柱・地割れ・突き出す岩）と島の縁の崩落 → `HeartShardScatter`（緑・金・赤の3つの浮遊石が
   地面から抜け出して浮かび、光の尾を引いて三方へ飛んでいく。石は `rock1.efkmodel`。`tools/art/heart_shatter_effect.py`。
   名前は昔の「欠片が散る」設定のまま）→ 揺れ → 島の底が崩れ落ちる →
...
- 教官の説明: 島を浮かせ続けるには浮遊石を戻すしかない。石はまだ力を放っていて、周りの魔物が荒れている。
  まずは一番近い草原から。
...
- 島の変化: 緑の浮遊石を島の底へ戻すと島の沈下が止まり、草木が戻る。
